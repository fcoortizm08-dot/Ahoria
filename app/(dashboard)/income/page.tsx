...
export default function IncomePage() {
  ...
  const fetchTransactions = useCallback(async () => {
    setLoading(true)
    try {
      const res  = await fetch(`/api/transactions?type=income&year=${year}&month=${month}`)
  ...
  useEffect(() => {
    ...
    supabase.from('categories').select('id, name, icon, color').eq('type', 'income').order('name')
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 px-5 py-4">
        <p className="text-xs text-emerald-600 mb-0.5">Total del mes</p>
      ...
      {showForm && (
        ...
        >
          <p className="text-sm font-semibold text-white mb-1">Nuevo ingreso</p>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center rounded-2xl border border-[#1e2d45] bg-[#0d1117] py-14 text-center">
          ...
          <p className="text-sm text-slate-500">Sin ingresos en {monthLabel}</p>
          <button
        ...
        <div className="space-y-2">
          {transactions.map((tx) => (
            ...
            >
              <div className="w-8 h-8 rounded-full bg-emerald-500/10 flex items-center justify-center text-sm flex-shrink-0">
                {tx.category?.icon ?? '💵'}
              </div>
