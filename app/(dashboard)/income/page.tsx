...
export default function IncomePage() {
  ...
  const fetchData = async () => {
    ...
    const [{ data: txs }, { data: cats }] = await Promise.all([
      supabase.from('transactions').select('*, category:categories(*)').eq('user_id', user.id).eq('type', 'income').is('deleted_at', null).gte('date', start).lte('date', end).order('date', { ascending: false }),
      supabase.from('categories').select('*').eq('type', 'income').or(`user_id.eq.${user.id},is_system.eq.true`),
    ])
    setTransactions(txs ?? [])
    setCategories(cats ?? [])
    setIsLoading(false)
    ...
    <div className="flex flex-col gap-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-400 text-sm mt-0.5">Total este mes: <span className="text-emerald-400 font-semibold">{formatCurrency(totalIncome)}</span></p>
        </div>
        <button
        ...
        <form onSubmit={handleSubmit} className="bg-[#0d1117] rounded-xl border border-[#1e2d45] p-5 flex flex-col gap-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="number" value={form.amount} onChange={e => setForm(f => ({ ...f, amount: e.target.value }))} required min="1" placeholder="100000" className="w-full bg-[#111827] border border-[#1e2d45] rounded-xl px-3.5 py-2.5 text-sm text-white placeholder:text-slate-600 outline-none focus:border-blue-500 transition-all" />
            </div>
            <div>
      ...
      <div className="bg-[#0d1117] rounded-xl border border-[#1e2d45] overflow-hidden">
        {isLoading ? (
          ...
          <div className="p-8 text-center text-slate-500 text-sm">No hay ingresos registrados este mes.</div>
        ) : (
